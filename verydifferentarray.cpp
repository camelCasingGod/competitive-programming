...
void solve() {
    ...
    for (int i = 0; i < n; i++) cin >> petya[i];
    for (int i = 0; i < m; i++) cin >> vasya[i];

    sort(petya, petya + n);
    sort(vasya, vasya + m);

    int vl = 0, vr = m - 1, pl = 0, pr = n - 1;
    int res = 0;
    
    while (pl <= pr) {
        if (max(abs(petya[pl] - vasya[vl]), abs(petya[pr] - vasya[vl])) > max(abs(petya[pl] - vasya[vr]), abs(petya[pr] - vasya[vr]))) {
            // left is greater
            if (abs(petya[pl] - vasya[vl]) > abs(petya[pr] - vasya[vl])) {
                res += abs(petya[pl] - vasya[vl]);
                ++pl;
            }
            else {
                res += abs(petya[pr] - vasya[vl]);
                --pr;
            }
            ++vl;
        } else {
            if (abs(petya[pl] - vasya[vr]) > abs(petya[pr] - vasya[vr])) {
                res += abs(petya[pl] - vasya[vr]);
                ++pl;
            }
            else {
                res += abs(petya[pr] - vasya[vr]);
                --pr;
            }
            --vr;
        }
